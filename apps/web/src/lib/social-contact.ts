export const PHONE_LOCAL = "01040200225";
export const PHONE_INTERNATIONAL = "201040200225";

export const EMAIL = "nozhasocialmedia@gmail.com";

export const FACEBOOK = "https://www.facebook.com/Sleepy.lingerie.HomeWear/";
export const TELEGRAM = "https://t.me/sleepylingerie";
export const TIKTOK = "#";
export const INSTAGRAM = "#";

export const WHATSAPP = `https://wa.me/${PHONE_INTERNATIONAL}`;

export const WHATSAPP_MESSAGE = encodeURIComponent(
  "\u0645\u0631\u062D\u0628\u0627\u064B\u060C \u0623\u0631\u064A\u062F \u0627\u0644\u0627\u0633\u062A\u0641\u0633\u0627\u0631 \u0639\u0646 \u0645\u0646\u062A\u062C\u0627\u062A SleepyWear"
);

export const WHATSAPP_LINK = `${WHATSAPP}?text=${WHATSAPP_MESSAGE}`;
